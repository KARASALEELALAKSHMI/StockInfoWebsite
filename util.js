...
class Utility {
  ...
  static AddEventListener(elementID, eventType, fnCallback) {
    const control = document.getElementById(elementID);
    control.addEventListener(eventType, fnCallback); // 1. Bind the event and the function callback to the control variable
  }
  ...
  static FetchFormValues(form) {
    const formElements = [...form.elements].filter(i => i.type !== 'submit'); // 2. Filter condition where element type is not submit
